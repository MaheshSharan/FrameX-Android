...
class CommandLabelParserTest {
    ...
    fun parse_contentInsert_extractsKeyAndValue() {
        ...
        assertEquals("sdk_game_target_fps", parsed.key)
    ...
    fun parse_settingsPut_extractsKeyAndValue() {
        ...
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("peak_refresh_rate", parsed.key)
        assertEquals("120", parsed.value)
    }
    ...
    fun parse_settingsPut_withQuotes_stripsQuotes() {
        ...
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("touch_sensitivity", parsed.key)
    ...
    fun parse_deviceConfigPut_extractsKeyAndValue() {
        ...
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("game_overlay", parsed.key)
        assertEquals("com.example.game mode=2,fps=120", parsed.value)
    }
    ...
    fun parse_pmTrimCaches_extractsVerbAndValue() {
        ...
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("pm trim-caches", parsed.key)
        assertEquals("4G", parsed.value)
    }

    @Test
    fun parse_amKillAll_extractsVerb() {
        val cmd = "am kill-all"
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("am kill-all", parsed.key)
        assertEquals("active", parsed.value)
    }
    ...
    fun parse_cmdThermalservice_extractsVerbAndArgs() {
        ...
        val parsed = CommandLabelParser.parse(cmd)
        assertEquals("cmd thermalservice", parsed.key)
        assertEquals("override-status 0", parsed.value)
    }
    ...
    fun formatLabel_formatsKeyValueProperly() {
        ...
        assertEquals("am kill-all", CommandLabelParser.formatLabel("am kill-all", "active"))
    }
